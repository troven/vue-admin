...
/**
 * Keycloak Adapter
 * original: https://github.com/dsb-norge/vue-keycloak-js/blob/master/src/index.js
 */
...
function init(config, watch, options) {
  ...
  function updateWatchVariables(isAuthenticated = false) {
    ...
    if (isAuthenticated) {
      ...
      watch.loadUserModel = keycloak.loadUserModel
    ...
    console.log('keycloak.refreshed: %o -> %o', isAuthenticated, watch)
    store.dispatch('user/login', watch.tokenParsed)
  }
}
